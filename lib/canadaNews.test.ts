...
assert.match(enNews[0].content, /04:15 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 04\.15 UTC/);
...
assert.match(enNews[1].content, /20:26:20 GMT/);
assert.match(trNews[1].content, /20\.26\.20 GMT/);
assert.match(enNews[1].content, /succeeded over HTTP\/2 and HTTP\/1\.1 after last hour’s failure/);
assert.match(trNews[1].content, /HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu; geçen saat HTTP\/2 akışı INTERNAL_ERROR/);
assert.match(enNews[1].content, /Last hour those official files failed over HTTP\/2 \(HTTP\/2 stream INTERNAL_ERROR\) and timed out over HTTP\/1\.1 \(no response within 40 seconds, 0 bytes\)/);
assert.match(trNews[1].content, /geçen saat HTTP\/2 akışı INTERNAL_ERROR ile başarısız olmuş ve HTTP\/1\.1 40 saniye içinde yanıt vermemişti \(0 bayt\)/);
assert.doesNotMatch(enNews[1].content, /Last hour those official files succeeded over HTTP\/2 and HTTP\/1\.1/);
assert.doesNotMatch(trNews[1].content, /Geçen saat bu resmi dosyalar HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı olmuştu/);
