...
assert.match(enNews[0].content, /07:10 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 07\.10 UTC/);
...
assert.match(enNews[1].content, /21:02:03 GMT/);
assert.match(trNews[1].content, /21\.02\.03 GMT/);
assert.match(enNews[1].content, /07:01:40 GMT/);
assert.match(trNews[1].content, /07\.01\.40 GMT/);
assert.match(enNews[1].content, /20:28:42 GMT/);
assert.match(trNews[1].content, /20\.28\.42 GMT/);
assert.match(enNews[1].content, /both succeeded over HTTP\/2 and HTTP\/1\.1/);
assert.match(enNews[1].content, /Last hour the official JSON succeeded over HTTP\/1\.1/);
assert.match(enNews[1].content, /HTTP\/2 stream INTERNAL_ERROR/);
assert.match(enNews[1].content, /official rounds HTML failed over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.match(trNews[1].content, /HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu/);
assert.match(trNews[1].content, /Geçen saat resmi JSON HTTP\/1\.1 üzerinden başarılı oldu/);
assert.match(trNews[1].content, /HTTP\/2 stream INTERNAL_ERROR/);
assert.match(trNews[1].content, /resmi tur HTML hem HTTP\/2 \(INTERNAL_ERROR\) hem HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız oldu/);
assert.doesNotMatch(enNews[1].content, /failed this hour over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
