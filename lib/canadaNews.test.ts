...
assert.match(enNews[0].content, /in negotiations|NATO Summit|4\.3 billion|TCE-Consultations|JETCO|420 million/);
assert.match(trNews[0].content, /müzakerede|NATO Zirvesi|4,3 milyar|TCE-Consultations|JETCO|420 milyon/);
assert.doesNotMatch(enNews[0].content, /\$3\.3B|3\.3 billion/i);
assert.doesNotMatch(trNews[0].content, /3,3 milyar/i);
assert.match(enNews[1].title, /Express Entry|French/i);
assert.match(trNews[1].title, /Express Entry|Fransızca/i);
assert.match(enNews[1].content, /NCLC 7|50,701|49\.2%|Francophone|30,267/);
assert.match(trNews[1].content, /NCLC 7|50\.701|%49,2|Frankofon|30\.267/);
