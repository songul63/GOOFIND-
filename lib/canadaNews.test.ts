...
assert.match(enNews[0].content, /20:10 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 20\.10 UTC/);
...
assert.match(enNews[1].content, /both succeeded over HTTP\/2 \(200\) and HTTP\/1\.1 \(200\)/);
assert.match(enNews[1].content, /Last hour HTTP\/2 succeeded \(200\) while HTTP\/1\.1 requests for both official files timed out after 50 seconds with 0 bytes received/);
assert.match(enNews[1].content, /20:10 UTC on 19 September 2026/);
assert.match(trNews[1].content, /hem HTTP\/2 \(200\) hem HTTP\/1\.1 \(200\) üzerinden başarılı oldu/);
assert.match(trNews[1].content, /Geçen saat HTTP\/2 başarılı oldu \(200\) ancak her iki resmi dosya için HTTP\/1\.1 istekleri 50 saniye sonra 0 baytla zaman aşımına uğradı/);
assert.match(trNews[1].content, /20\.10 UTC/);
assert.doesNotMatch(enNews[1].content, /Last hour they failed over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.doesNotMatch(enNews[1].content, /official files could not be re-fetched this hour/);
assert.doesNotMatch(trNews[1].content, /Geçen saat her ikisi de hem HTTP\/2 \(INTERNAL_ERROR\) hem HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız oldu/);
