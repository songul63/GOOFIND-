...
assert.match(enNews[0].content, /in negotiations|NATO Summit|Bolat|Sidhu|JETCO|TCE-Consultations/);
assert.match(trNews[0].content, /müzakerede|NATO Zirvesi|Bolat|Sidhu|JETCO|TCE-Consultations/);
assert.doesNotMatch(enNews[0].content, /\$3\.3B|3\.3 billion/i);
assert.doesNotMatch(trNews[0].content, /3,3 milyar/i);
assert.match(enNews[0].content, /4,650|flat-rolled|tugboats|Stewart|Tuzcu/);
assert.match(trNews[0].content, /4\.650|yassı haddelenmiş|römorkör|Stewart|Tuzcu/);
assert.match(enNews[1].title, /Express Entry|military/i);
assert.match(trNews[1].title, /Express Entry|askeri/i);
assert.match(enNews[1].content, /FSMA|NOC 40042|42102|43204|ten years|three years|Canadian Forces Recruiting Group/);
assert.match(trNews[1].content, /FSMA|NOC 40042|42102|43204|on yıl|üç yıl|İşe Alım Grubu/);
