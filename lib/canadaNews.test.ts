...
assert.match(enNews[0].content, /in negotiations|NATO Summit|4\.3 billion|TCE-Consultations|JETCO|lentils|Air Transport/);
assert.match(trNews[0].content, /müzakerede|NATO Zirvesi|4,3 milyar|TCE-Consultations|JETCO|mercimek|Hava Ulaştırma/);
...
assert.match(enNews[1].title, /Express Entry|health/i);
assert.match(trNews[1].title, /Express Entry|sağlık/i);
assert.match(enNews[1].content, /12 months|31301|nurse practitioners|19,200|18\.6%/);
assert.match(trNews[1].content, /12 aylık|31301|hemşire|19\.200|%18,6/);
assert.doesNotMatch(enNews[1].content, /3,500|CRS 475/);
assert.doesNotMatch(trNews[1].content, /3\.500|CRS 475/);
