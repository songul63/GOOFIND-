...
assert.match(enNews[0].content, /19:20 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 19\.20 UTC/);
assert.match(enNews[1].content, /13:53:31 GMT/);
assert.match(trNews[1].content, /13\.53\.31 GMT/);
assert.match(enNews[1].content, /16:34:10 GMT/);
assert.match(trNews[1].content, /16\.34\.10 GMT/);
...
assert.match(enNews[1].content, /both succeeded over HTTP\/2 \(200\)/);
assert.match(enNews[1].content, /HTTP\/1\.1 requests for both official files timed out after 50 seconds with 0 bytes received/);
assert.match(enNews[1].content, /Last hour they failed over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.match(enNews[1].content, /19:20 UTC on 19 September 2026/);
assert.match(trNews[1].content, /HTTP\/2 üzerinden başarılı oldu \(200\)/);
assert.match(trNews[1].content, /HTTP\/1\.1 istekleri 50 saniye sonra 0 baytla zaman aşımına uğradı/);
...
assert.match(trNews[1].content, /19\.20 UTC/);
assert.doesNotMatch(enNews[1].content, /failed again over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.doesNotMatch(enNews[1].content, /official files could not be re-fetched this hour/);
assert.doesNotMatch(trNews[1].content, /yine hem HTTP\/2 \(INTERNAL_ERROR\) hem HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız oldu/);
assert.doesNotMatch(trNews[1].content, /resmi dosyalar bu saat yeniden indirilemedi/);
