...
assert.match(enNews[0].content, /05:19 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 05\.19 UTC/);
...
assert.match(enNews[1].content, /again succeeded over HTTP\/2 and HTTP\/1\.1/);
assert.match(trNews[1].content, /yine HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu/);
assert.match(enNews[1].content, /Last hour those official files also succeeded over HTTP\/2 and HTTP\/1\.1 after the previous hour’s failure/);
assert.match(trNews[1].content, /Geçen saat bu resmi dosyalar da HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı olmuştu \(bir önceki saatin başarısızlığından sonra\)/);
assert.doesNotMatch(enNews[1].content, /This hour official ee_rounds_123_en.json and the Express Entry rounds HTML page succeeded over HTTP\/2 and HTTP\/1\.1 after last hour’s failure/);
assert.doesNotMatch(trNews[1].content, /Bu saatte resmi ee_rounds_123_en.json ve Express Entry tur HTML sayfası HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu; geçen saat HTTP\/2 akışı INTERNAL_ERROR/);
