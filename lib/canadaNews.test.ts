...
assert.match(enNews[0].content, /12:20 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 12\.20 UTC/);
...
assert.match(enNews[1].content, /both failed this hour over HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
...
assert.match(enNews[1].content, /could not be re-fetched this hour/);
assert.match(trNews[1].content, /bu saat yeniden indirilemedi/);
