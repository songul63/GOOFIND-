...
assert.match(enNews[0].content, /11:15 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 11\.15 UTC/);
...
assert.match(enNews[1].content, /both succeeded this hour over HTTP\/2 and HTTP\/1\.1/);
assert.match(trNews[1].content, /bu saatte hem HTTP\/2 hem HTTP\/1\.1 üzerinden başarılı oldu/);
assert.match(enNews[1].content, /Last hour they both failed over HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.match(trNews[1].content, /Geçen saat her ikisi HTTP\/2 \(INTERNAL_ERROR\) ve HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız olmuştu/);
assert.match(enNews[1].content, /Official files were re-fetched this hour/);
assert.match(trNews[1].content, /Resmi dosyalar bu saat yeniden indirildi/);
...
assert.doesNotMatch(enNews[1].content, /failed this hour over both HTTP\/2/);
