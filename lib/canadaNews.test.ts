...
assert.match(enNews[0].content, /13:16 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 13\.16 UTC/);
...
assert.match(enNews[1].content, /successfully re-fetched this hour after last hour’s failures/);
assert.match(trNews[1].content, /geçen saatin HTTP\/2 \(INTERNAL_ERROR\) ve HTTP\/1\.1 \(zaman aşımı\) başarısızlıklarından sonra bu saat yeniden indirildi/);
