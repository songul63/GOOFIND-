...
assert.match(enNews[0].content, /00:15 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 00\.15 UTC/);
...
assert.match(enNews[1].content, /succeeded over both HTTP\/2 and HTTP\/1\.1/);
assert.match(trNews[1].content, /hem HTTP\/2 hem HTTP\/1\.1 üzerinden başarılı/);
assert.match(enNews[1].content, /HTML page also loaded this hour over both HTTP\/2 and HTTP\/1\.1/);
assert.match(trNews[1].content, /HTML sayfası da bu saatte hem HTTP\/2 hem HTTP\/1\.1 üzerinden yüklendi/);
...
assert.doesNotMatch(enNews[1].content, /Last hour those official files failed/);
assert.doesNotMatch(trNews[1].content, /Geçen saat bu resmi dosyalar başarısız oldu/);
