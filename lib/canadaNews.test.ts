...
assert.match(enNews[0].content, /Current status: Closed|consultation page still lists|conducted public consultations|what-we-heard report has not yet been published|in negotiations|NATO Summit|4\.3 billion|TCE-Consultations/);
assert.match(trNews[0].content, /Current status: Closed|conducted public consultations|henüz yayımlanmamış|müzakerede|NATO Zirvesi|4,3 milyar|TCE-Consultations/);
...
assert.match(enNews[1].content, /no #445 has been published|no round after #444/i);
assert.match(trNews[1].content, /#445 yayımlanmamıştır|#444.ten sonra/i);
assert.match(enNews[0].content, /14:18 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 14\.18 UTC/);
assert.match(enNews[1].content, /13:53:31 GMT/);
assert.match(trNews[1].content, /13\.53\.31 GMT/);
assert.match(enNews[1].content, /02:00:58 GMT/);
assert.match(trNews[1].content, /02\.00\.58 GMT/);
assert.match(enNews[1].content, /11:09:13 GMT/);
assert.match(trNews[1].content, /11\.09\.13 GMT/);
assert.match(enNews[1].content, /07:01:40 GMT/);
assert.match(trNews[1].content, /07\.01\.40 GMT/);
assert.match(enNews[1].content, /successfully re-fetched this hour/);
assert.match(trNews[1].content, /bu saat yeniden indirildi/);
assert.match(enNews[1].content, /Sequential HTTP\/2 and HTTP\/1\.1 requests both returned 200/);
assert.match(trNews[1].content, /Sıralı HTTP\/2 ve HTTP\/1\.1 istekleri her ikisi de 200 döndü/);
assert.match(enNews[1].content, /curl HTTP\/2 request still failed this hour with INTERNAL_ERROR/);
assert.match(trNews[1].content, /curl HTTP\/2 isteği bu saatte hâlâ INTERNAL_ERROR verdi/);
assert.match(enNews[1].content, /curl HTTP\/1\.1 request still timed out/);
assert.match(trNews[1].content, /curl HTTP\/1\.1 isteği hâlâ zaman aşımına uğradı/);
assert.match(enNews[1].content, /official files themselves were retrieved and confirmed/);
assert.match(trNews[1].content, /resmi dosyaların kendisi alındı ve teyit edildi/);
assert.match(enNews[1].content, /official JSON was re-fetched this hour/);
assert.match(trNews[1].content, /resmi JSON bu saat yeniden indirildi/);
assert.match(enNews[1].content, /Prince Edward Island PNP story \(195 candidates/);
assert.match(trNews[1].content, /Prince Edward Island PNP haberi \(öncelikli sektörlerde 195 aday\)/);
assert.doesNotMatch(enNews[1].content, /both failed this hour over HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.doesNotMatch(trNews[1].content, /bu saatte hem HTTP\/2 \(INTERNAL_ERROR\) hem HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız oldu/);
assert.doesNotMatch(enNews[1].content, /both succeeded this hour over HTTP\/2 and HTTP\/1\.1/);
assert.doesNotMatch(trNews[1].content, /bu saatte hem HTTP\/2 hem HTTP\/1\.1 üzerinden başarılı oldu/);
assert.doesNotMatch(enNews[1].content, /after last hour’s failures/);
assert.doesNotMatch(trNews[1].content, /geçen saatin HTTP\/2 \(INTERNAL_ERROR\) ve HTTP\/1\.1 \(zaman aşımı\) başarısızlıklarından sonra/);
