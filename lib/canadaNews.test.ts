...
assert.match(enNews[0].content, /01:05 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 01\.05 UTC/);
assert.match(enNews[1].content, /13:53:31 GMT/);
assert.match(trNews[1].content, /13\.53\.31 GMT/);
assert.match(enNews[1].content, /00:03:32 GMT/);
assert.match(trNews[1].content, /00\.03\.32 GMT/);
...
assert.match(enNews[1].content, /failed over HTTP\/2 \(HTTP\/2 stream INTERNAL_ERROR\) and timed out over HTTP\/1\.1 \(no response within 45 seconds\)/);
assert.match(trNews[1].content, /HTTP\/2 üzerinden başarısız oldu \(HTTP\/2 akışı INTERNAL_ERROR\) ve HTTP\/1\.1 üzerinden zaman aşımına uğradı \(45 saniye içinde yanıt yok\)/);
assert.match(enNews[1].content, /Last hour those official files succeeded/);
assert.match(trNews[1].content, /Geçen saat bu resmi dosyalar başarılı oldu/);
assert.doesNotMatch(enNews[1].content, /succeeded over both HTTP\/2 and HTTP\/1\.1/);
assert.doesNotMatch(trNews[1].content, /hem HTTP\/2 hem HTTP\/1\.1 üzerinden başarılı/);
assert.doesNotMatch(enNews[1].content, /HTML page also loaded this hour over both HTTP\/2 and HTTP\/1\.1/);
assert.doesNotMatch(trNews[1].content, /HTML sayfası da bu saatte hem HTTP\/2 hem HTTP\/1\.1 üzerinden yüklendi/);
