...
assert.match(enNews[1].content, /official rounds JSON loaded this hour/i);
assert.match(trNews[1].content, /Bu saatte yüklenen resmi tur JSON/i);
assert.doesNotMatch(enNews[1].content, /timed out this hour/i);
assert.doesNotMatch(trNews[1].content, /zaman aşımına uğradı/i);
assert.match(enNews[0].content, /21:00 UTC on 17 September 2026/);
assert.match(trNews[0].content, /17 Eylül 2026 21\.00 UTC/);
assert.match(enNews[0].content, /conducted public consultations|now uses past tense/i);
assert.match(trNews[0].content, /conducted public consultations|geçmiş zaman/i);
assert.doesNotMatch(enNews[0].content, /still uses present tense/i);
assert.doesNotMatch(trNews[0].content, /hâlâ şimdiki zaman/i);
