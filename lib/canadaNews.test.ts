...
assert.match(enNews[0].content, /02:09 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 02\.09 UTC/);
...
assert.match(enNews[1].content, /last hour’s failure \(HTTP\/2 stream INTERNAL_ERROR and HTTP\/1\.1 timeouts with 0 bytes\)/);
assert.match(trNews[1].content, /geçen saat HTTP\/2 akışı INTERNAL_ERROR ve HTTP\/1\.1 zaman aşımları \(0 bayt\)/);
assert.doesNotMatch(enNews[1].content, /failed over HTTP\/2 \(HTTP\/2 stream INTERNAL_ERROR\) and timed out over HTTP\/1\.1 \(no response within 45 seconds\)/);
assert.doesNotMatch(trNews[1].content, /HTTP\/2 üzerinden başarısız oldu \(HTTP\/2 akışı INTERNAL_ERROR\) ve HTTP\/1\.1 üzerinden zaman aşımına uğradı \(45 saniye içinde yanıt yok\)/);
assert.doesNotMatch(enNews[1].content, /Last hour those official files succeeded/);
assert.doesNotMatch(trNews[1].content, /Geçen saat bu resmi dosyalar başarılı oldu/);
