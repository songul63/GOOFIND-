...
assert.match(enNews[0].content, /06:10 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 06\.10 UTC/);
...
assert.match(enNews[1].content, /21:02:06 GMT/);
assert.match(trNews[1].content, /21\.02\.06 GMT/);
assert.match(enNews[1].content, /20:28:42 GMT/);
assert.match(trNews[1].content, /20\.28\.42 GMT/);
assert.match(enNews[1].content, /succeeded over HTTP\/1\.1/);
assert.match(enNews[1].content, /HTTP\/2 stream INTERNAL_ERROR/);
assert.match(enNews[1].content, /failed this hour over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.match(trNews[1].content, /HTTP\/1\.1 üzerinden başarılı oldu/);
assert.match(trNews[1].content, /HTTP\/2 stream INTERNAL_ERROR/);
assert.match(trNews[1].content, /hem HTTP\/2 \(INTERNAL_ERROR\) hem HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız oldu/);
assert.match(enNews[1].content, /Last hour those official files both succeeded over HTTP\/2 and HTTP\/1\.1/);
assert.match(trNews[1].content, /Geçen saat bu resmi dosyalar HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı olmuştu/);
assert.match(enNews[1].content, /last hour’s successful fetch/);
assert.match(trNews[1].content, /geçen saatin başarılı indirmesinden/);
assert.doesNotMatch(enNews[1].content, /again succeeded over HTTP\/2 and HTTP\/1\.1/);
assert.doesNotMatch(trNews[1].content, /yine HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu/);
