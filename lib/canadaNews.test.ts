...
assert.match(enNews[0].content, /in negotiations|NATO Summit|4\.3 billion|JETCO|420 million|TCE-Consultations/);
assert.match(trNews[0].content, /müzakerede|NATO Zirvesi|4,3 milyar|JETCO|420 milyon|TCE-Consultations/);
...
assert.match(trNews[1].title, /Express Entry|ticaret/i);
assert.match(enNews[1].content, /12 months|NOC 70010|72200|72300|in Canada or abroad|does not need to be continuous/);
assert.match(trNews[1].content, /12 aylık|NOC 70010|72200|72300|Kanada’da veya yurt dışında|kesintisiz olması/);
assert.match(enNews[2].title, /Anatolia Fest/);
assert.match(trNews[2].title, /Anatolia Fest/);
assert.match(enNews[2].content, /Sankofa Square|kutuphane\.ca|baklava|dance/i);
assert.match(trNews[2].content, /Sankofa Square|kutuphane\.ca|baklava|dans/i);
