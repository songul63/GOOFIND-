...
assert.match(enNews[0].content, /18:20 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 18\.20 UTC/);
...
assert.match(enNews[1].content, /same as last hour/);
assert.match(trNews[1].content, /geçen saatle aynı/);
assert.match(enNews[1].content, /17:58:56 GMT/);
assert.match(trNews[1].content, /17\.58\.56 GMT/);
assert.match(enNews[1].content, /07:01:40 GMT/);
assert.match(trNews[1].content, /07\.01\.40 GMT/);
assert.match(enNews[1].content, /Prince Edward Island PNP|195 candidates/);
assert.match(trNews[1].content, /PEI PNP|195 aday/);
assert.match(enNews[1].content, /failed again over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.match(enNews[1].content, /Last hour they also failed over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.match(enNews[1].content, /15:09 UTC on 19 September 2026/);
assert.match(enNews[1].content, /official files could not be re-fetched this hour/);
assert.match(trNews[1].content, /yine hem HTTP\/2 \(INTERNAL_ERROR\) hem HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız oldu/);
assert.match(trNews[1].content, /Geçen saat her ikisi de hem HTTP\/2 \(INTERNAL_ERROR\) hem HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız oldu/);
assert.match(trNews[1].content, /15\.09 UTC/);
assert.match(trNews[1].content, /resmi dosyalar bu saat yeniden indirilemedi/);
assert.doesNotMatch(enNews[1].content, /This hour official ee_rounds_123_en.json and the official Express Entry rounds HTML page both succeeded over HTTP\/2 and HTTP\/1\.1/);
assert.doesNotMatch(enNews[1].content, /Last hour they both succeeded over HTTP\/2 and HTTP\/1\.1/);
assert.doesNotMatch(trNews[1].content, /Bu saatte resmi ee_rounds_123_en.json ve resmi Express Entry tur HTML sayfası HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu/);
assert.doesNotMatch(trNews[1].content, /Geçen saat her ikisi HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu/);
