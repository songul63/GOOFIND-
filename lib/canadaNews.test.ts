...
assert.match(enNews[0].content, /16:22 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 16\.22 UTC/);
...
assert.match(enNews[1].content, /failed over both HTTP\/2 \(INTERNAL_ERROR\) and HTTP\/1\.1 \(timeout\)/);
assert.match(enNews[1].content, /Last hour they both succeeded over HTTP\/2 and HTTP\/1\.1/);
...
assert.match(trNews[1].content, /Geçen saat her ikisi HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu/);
