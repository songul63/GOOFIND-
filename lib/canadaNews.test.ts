...
assert.match(enNews[0].content, /10:08 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 10\.08 UTC/);
...
assert.match(trNews[1].content, /bu saatte hem HTTP\/2 \(INTERNAL_ERROR\) hem HTTP\/1\.1 \(zaman aşımı\) üzerinden başarısız oldu/);
assert.match(enNews[1].content, /official files could not be re-fetched this hour/i);
assert.match(trNews[1].content, /resmi dosyalar bu saat yeniden indirilemedi/i);
...
assert.match(trNews[1].content, /Geçen saat her ikisi HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı olmuştu/);
assert.doesNotMatch(enNews[1].content, /both succeeded over HTTP\/2 and HTTP\/1\.1\. Last hour they also both succeeded/);
assert.doesNotMatch(trNews[1].content, /Geçen saat de her ikisi HTTP\/2 ve HTTP\/1\.1 üzerinden başarılı oldu/);
