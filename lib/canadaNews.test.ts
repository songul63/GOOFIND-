...
assert.match(enNews[0].content, /15:09 UTC on 19 September 2026/);
assert.match(trNews[0].content, /19 Eylül 2026 15\.09 UTC/);
...
assert.match(enNews[1].content, /Parallel curl HTTP\/2 and HTTP\/1\.1 requests also both returned 200 this hour/);
assert.match(trNews[1].content, /Paralel curl HTTP\/2 ve HTTP\/1\.1 istekleri de bu saatte her ikisi 200 döndü/);
...
assert.doesNotMatch(enNews[1].content, /INTERNAL_ERROR/);
assert.doesNotMatch(trNews[1].content, /INTERNAL_ERROR/);
assert.doesNotMatch(enNews[1].content, /timed out/);
assert.doesNotMatch(trNews[1].content, /zaman aşımına uğradı/);
